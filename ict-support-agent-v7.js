...
const ticketFields = {
  ...
  priorityReason: document.querySelector("#ticketPriorityReason"),
...
function addPriorityBadgeMessage() {
  ...
  details.textContent = `Impact: ${state.impact} | Urgentie: ${state.urgency} | Prioriteit: ${state.priority.level} ${state.priority.label} | ${state.priorityReason}`;
...
function createTicket() {
  ...
  const description = [
    ...
    "",
    "Impact/urgentie:",
    `- Kun je nog werken: ${formatWorkStatus()}`,
    ...
    `- Reden van classificatie: ${state.priorityReason}`,
    "",
  ...
  ticketFields.title.textContent = currentTicketJson.title;
  ticketFields.category.textContent = currentTicketJson.category;
  ticketFields.impact.textContent = currentTicketJson.impact;
  ticketFields.urgency.textContent = currentTicketJson.urgency;
  ticketFields.priority.innerHTML = "";
  ticketFields.priority.appendChild(createPriorityBadge());
  ticketFields.priorityReason.textContent = state.priorityReason;
  ticketFields.description.textContent = currentTicketJson.description;
  ticketFields.source.textContent = currentTicketJson.source;
  ...
  addMessage(
    "agent",
    `Het TOPdesk-conceptticket is voorbereid.\n\nPrioriteit: ${currentTicketJson.priority}\nImpact: ${currentTicketJson.impact}\nUrgentie: ${currentTicketJson.urgency}\n${currentTicketJson.priorityReason}\n\nControleer het ticket rechts in beeld. Start een nieuwe melding als je nog iets anders wilt registreren.`
  );
...
function downloadTicketJson() {
  ...
  const blob = new Blob([JSON.stringify(currentTicketJson, null, 2)], {
...
downloadTicketButton.addEventListener("click", downloadTicketJson);
